...
public class HBaseMr {
			...
			/*插入数据*/
				...
				p1.add(colf.getBytes(), col.getBytes(),	
						("The Apache Hadoop software library "
								+ "is a framework").getBytes());
				lp.add(p1);
				Put p2 = new Put(Bytes.toBytes("2"));
				p2.add(colf.getBytes(),col.getBytes(),
						("The common utilities that support the "
								+ "other Hadoop modules").getBytes());
				lp.add(p2);
	...
	public static class MyMapper 
		...
		protected void map(ImmutableBytesWritable key, Result value,
			...
			//获取一行数据中的colf：col
			String words = Bytes.toString(value.getValue(Bytes.
					toBytes(colf), Bytes.toBytes(col)));
	...
	public static class MyReducer extends
		...
		protected void reduce(Text key, Iterable<IntWritable> values,
			...
			// 封装数据
			put.add(Bytes.toBytes(colf), Bytes.toBytes(col),Bytes.
